...
let planetBtn = document.getElementById('planet')
...
function planet(){
    ...
    let planetDraw = setInterval(() => {
        let planetSize = Math.random() * 300
        let planetRot = Math.random() * 180
        let planetPosTop = Math.random() * window.innerHeight
        let planetPosLeft = Math.random() * window.innerWidth
        setPropet(planetSize, planetPosTop, planetPosLeft, planetRot)    
    }, 300);

    setTimeout(() => {
        clearInterval(planetDraw)
    }, 10000);
}
...
function setPropet(size: number, posTop: number, posLeft: number, rot: number){
    let wrp = document.getElementById('wrapper')

    let planetElm = document.createElement('div')
    ...
    if(wrp && planetElm){
        ...
        planetElm.innerHTML = `<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                                    viewBox="0 0 502.74 502.74" xml:space="preserve">
                                    <g>
                            <path style="fill:none;" d="M395.431,251.37c0,0.962-0.018,1.919-0.036,2.876C395.418,253.291,395.431,252.332,395.431,251.37z"/>
                            <path style="fill:#` + color1 + `;" d="M434.563,175.558c-0.695-1.68-5.984-4.57-20.336-4.57c-8.906,0-21.297,1.109-38.25,4.313
                            ...
                            <path style="fill:#` + color1 + `;" d="M279.66,110.144c0.924,0.184,1.844,0.379,2.761,0.58
                                C281.504,110.523,280.584,110.328,279.66,110.144z"/>
                            <path style="fill:#` + color4 + `;" d="M286.315,111.634c1.041,0.26,2.078,0.53,3.111,0.813
                                C288.394,112.165,287.357,111.894,286.315,111.634z"/>
                            <path style="fill:#` + color4 + `;" d="M283.012,110.853c0.98,0.22,1.957,0.45,2.929,0.69C284.968,111.304,283.992,111.072,283.012,110.853
                                z"/>
                            <path style="fill:#` + color4 + `;" d="M395.394,254.246c0.019-0.957,0.036-1.915,0.036-2.876c0-0.786-0.022-1.565-0.035-2.348
                            ...
                            <path style="fill:#` + color4 + `;" d="M336.357,135.08c-13.912-10.177-29.717-17.917-46.79-22.595
                                c17.057,4.682,32.842,12.433,46.744,22.606C336.326,135.087,336.341,135.083,336.357,135.08z"/>
                            <path style="fill:#` + color4 + `;" d="M272.829,108.955c0.812,0.121,1.619,0.254,2.427,0.388
                                C274.448,109.209,273.641,109.076,272.829,108.955z"/>
                            <path style="fill:#` + color4 + `;" d="M269.35,108.479c0.761,0.094,1.519,0.201,2.276,0.308
                                C270.869,108.68,270.112,108.573,269.35,108.479z"/>
                            <path style="fill:#` + color4 + `;" d="M265.837,108.085c0.706,0.07,1.407,0.153,2.11,0.233
                                C267.244,108.238,266.542,108.155,265.837,108.085z"/>
                            <path style="fill:#` + color4 + `;" d="M262.283,107.775c0.646,0.048,1.289,0.109,1.934,0.166
                                C263.572,107.885,262.929,107.824,262.283,107.775z"/>
                            <path style="fill:#` + color4 + `;" d="M255.08,107.416c0.45,0.011,0.898,0.034,1.347,0.05C255.978,107.45,255.53,107.427,255.08,107.416z"
                                />
                            <path style="fill:#` + color4 + `;" d="M258.699,107.552c0.573,0.029,1.142,0.07,1.713,0.105
                                C259.841,107.622,259.271,107.581,258.699,107.552z"/>
                            <path style="fill:#` + color4 + `;" d="M276.269,109.511c0.867,0.151,1.73,0.313,2.592,0.479
                                C277.999,109.825,277.136,109.662,276.269,109.511z"/>
                            <path style="fill:#` + color4 + `;" d="M128.965,327.129c-1.196-1.927-2.35-3.883-3.456-5.87c1.105,1.987,2.255,3.944,3.449,5.871
                                C128.961,327.13,128.963,327.13,128.965,327.129z"/>
                            <path style="fill:#` + color4 + `;" d="M119.786,309.797c0.34,0.765,0.688,1.524,1.041,2.282
                                C120.474,311.322,120.126,310.562,119.786,309.797z"/>
                            <path style="fill:#` + color4 + `;" d="M122.572,315.693c0.402,0.804,0.811,1.603,1.228,2.399
                                C123.383,317.297,122.974,316.497,122.572,315.693z"/>
                            <path style="fill:#` + color4 + `;" d="M124.03,318.53c0.446,0.844,0.899,1.684,1.361,2.518C124.929,320.214,124.477,319.374,124.03,318.53
                                z"/>
                            <path style="fill:#` + color4 + `;" d="M121.152,312.775c0.368,0.78,0.744,1.555,1.126,2.327
                                C121.896,314.33,121.521,313.554,121.152,312.775z"/>
                            <path style="fill:#` + color4 + `;" d="M118.476,306.758c0.317,0.76,0.643,1.514,0.973,2.267
                                C119.119,308.272,118.793,307.517,118.476,306.758z"/>
                            <path style="fill:#` + color3 + `;" d="M189.031,289.77c0-10.785,1.195-21.29,3.443-31.398c-40.088,19.702-68.488,38.078-74.006,48.367
                                ...
                                />
                            <path style="fill:#` + color3 + `;" d="M336.311,135.091c-13.902-10.173-29.687-17.924-46.744-22.606c-0.047-0.013-0.094-0.025-0.141-0.038
                            ...
                            <path style="fill:` + color2 + `;" d="M500.446,148.229v-0.004c-9.359-22.59-43.781-31.23-99.531-25.031
                            ...
                            <path style="fill:#` + color2 + `;" d="M231.253,202.447C99.035,257.022,29.891,319.706,38.524,340.621
        ...
        wrp.appendChild(planetElm)

        planetElm.style.width = size + 'px'
        planetElm.style.height = size + '100px'
        
        planetElm.style.position = 'absolute'

        planetElm.style.top = posTop + 'px'
        planetElm.style.left = posLeft + 'px'

        planetElm.style.transform = 'rotate(' + rot + '40deg)'
    }
}